...
function applyEffectPreset(name) {
  ...
  // Pre-warm continuous effects to steady state so the preset shows its
  // real look instantly instead of trickling in over several seconds.
  // (Renders are unaffected — they still start from a clean reset.)
  warmUpSimulation();
...
/** Run the sim silently to steady state (continuous mode only). */
function warmUpSimulation() {
  if (cfg.emitterMode !== 'continuous') return;
  const warm = Math.min(cfg.lifetime || 60, 240);
  for (let i = 0; i < warm; i++) {
    tickEmitter();
    // Let the last few ticks build trail history + a visible frame
    if (i >= warm - 12) renderFrame();
  }
}
